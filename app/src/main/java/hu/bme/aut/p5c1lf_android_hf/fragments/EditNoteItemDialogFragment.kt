...
class EditNoteItemDialogFragment : DialogFragment() {
    interface EditNoteItemDialogListener {
        fun onNoteItemCreated(newItem: NoteItem)
        fun getNoteDetails() :NoteItem
    }

    private lateinit var listener: EditNoteItemDialogListener

    lateinit var binding: DialogNewNoteItemBinding

    override fun onAttach(context: Context) {
        super.onAttach(context)
        listener = context as? EditNoteItemDialogListener
            ?: throw RuntimeException("Activity must implement the EditNoteItemDialogListener interface!")
    }

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        binding = DialogNewNoteItemBinding.inflate(LayoutInflater.from(context))
        binding.etTitle.setText(listener.getNoteDetails().title)
        binding.etNote.setText(listener.getNoteDetails().note)
        return AlertDialog.Builder(requireContext())
            .setTitle(R.string.edit_note_item)
